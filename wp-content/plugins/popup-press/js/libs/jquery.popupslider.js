...
(function ($) {
  ...
  $.popupslider = function(el, options) {
    var slider = $(el);

    // making variables public
    slider.vars = $.extend({}, $.popupslider.defaults, options);
    ...
    methods = {
        ...
        setup: function() {
          ...
          if(!msGesture){
              slider.slides.on(eventType, function(e){
                ...
                } else if (!$(slider.vars.asNavFor).data('popupslider').animating && !$slide.hasClass(namespace + "active-slide")) {
              ...
              slider.slides.each(function (){
                  ...
                  that.addEventListener("MSGestureTap", function (e){
                      ...
                      var $slide = $(this),
                          target = $slide.index();
                      if (!$(slider.vars.asNavFor).data('popupslider').animating && !$slide.hasClass('active')) {
                          slider.direction = (slider.currentItem < target) ? "next" : "prev";
                          slider.flexAnimate(target, slider.vars.pauseOnAction, false, true, true);
                      }
                  });
      ...
      sync: function(action) {
        var $obj = $(slider.vars.sync).data("popupslider"),
    ...
    slider.flexAnimate = function(target, pause, override, withSync, fromNav) {
      ...
      if (!slider.animating && (slider.canAdvance(target, fromNav) || override) && slider.is(":visible")) {
        if (asNav && withSync) {
          var master = $(slider.vars.asNavFor).data('popupslider');
        ...
        if (type === "init") {
          if (!touch) {
            ...
            if (slider.vars.fadeFirstSlide == false) {
              slider.slides.css({ "opacity": 0, "display": "none", "zIndex": 1 }).eq(slider.currentSlide).css({"zIndex": 2, "display": "block"}).css({"opacity": 1, "display": "block"});
            } else {
    ...
    slider.destroy = function() {
      ...
      slider.find('.clone').remove(); // Remove any popupslider clones
      ...
      slider.removeData('popupslider'); // Remove data
    }
  ...
  $.popupslider.defaults = {
    ...
    // Special properties
    controlsContainer: "",          //{UPDATED} jQuery Object/Selector: Declare which container the navigation elements should be appended too. Default container is the FlexSlider element. Example use would be $(".popupslider-container"). Property is ignored if given element is not found.
  ...
  $.fn.popupslider = function(options) {
    ...
    if (typeof options === "object") {
      ...
      if ( ( $slides.length === 1 && options.allowOneSlide === true ) || $slides.length === 0 ) {
        ...
        } else if ($this.data('popupslider') === undefined) {
          new $.popupslider(this, options);
        }
      });
    } else {
      // Helper strings to quickly perform functions on the slider
      var $slider = $(this).data('popupslider');
      switch (options) {
