	...
	ChatX.prototype.add_msg = function( chat_id, raw_msg, is_new ) {
		...
		if( !this.last_msg ) {
			...
			if( msg_data.self_msg ) {
				cls.push( 'scx-you-start' );
			} else {
				cls.push( 'scx-other-start' );
			}

		} else if( !this.last_msg.self_msg && msg_data.self_msg ) {
			cls.push( 'scx-you-start' );

			prev_msg_class = ' scx-other-end';

		// Group other messages
		} else if( this.last_msg.self_msg && !msg_data.self_msg ) {
			cls.push( 'scx-other-start' );

			prev_msg_class = ' scx-you-end';
		}

		// Set previous message
		if( prev_msg_class ) {
			D.getElementById('SCX-msg-' + this.last_msg.id ).className += prev_msg_class;
		}
