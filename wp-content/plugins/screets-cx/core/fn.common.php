...
function scx__( $id ) {
	...
	$str = $ChatX->opts->getOption( $id );

	if( has_filter( 'wpml_translate_single_string' ) ) {
		return apply_filters( 'wpml_translate_single_string', $str, SCX_NAME, $id );
 ...
 * @return string $value
 */
function fn_scx_sanitize( $value, $line_break = false ) {
 ...
 */
function fn_scx_parse_email( $str ) {
 ...
 * @return string URL
 */
function fn_scx_current_page_url() {
 ...
 * @return string URL
 */
function fn_scx_current_domain() {

	$url = fn_scx_current_page_url();
 ...
 * @return string $desc
 */
function fn_scx_admin_desc( $str, $new_line = false ) {
	$r = '';
