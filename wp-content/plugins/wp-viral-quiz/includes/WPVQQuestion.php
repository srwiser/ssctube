...
class WPVQQuestion {
	 ...
	 * @return [type] [description]
	 */
	public function getParsedView($quizType, $questionNum) {
		$path = plugin_dir_path( __FILE__ ) . '/../views/WPVQAddQuestion.'.$quizType.'.answer.append.php';
		return $this->parseAddAnswer($path, $questionNum, $quizType);
	}

	private function parseAddAnswer($templatePath, $questionNum, $quizType) 
	{
		$finalTemplate 	= '';
		$quiz 			= new $quizType();
