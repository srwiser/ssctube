...
if ( ! class_exists( 'WPSEO_Local_Admin' ) ) {
	...
	class WPSEO_Local_Admin {
		...
		public function __construct() {
			...
			// Remove email-support help-center item.
			add_filter( 'wpseo_help_center_items', array( $this, 'remove_email_support_tab' ) );
		}
		...
		/**
		 * Removes the email support tab from the wpseo help center
		 *
		 * @param $help_center_items
		 *
		 * @return array WPSEO_Help_Center_Item[]
		 */
		public function remove_email_support_tab( $help_center_items ) {
			foreach ( $help_center_items as $index => $help_center_item ) {
				if ( ! is_a( $help_center_item, 'WPSEO_Help_Center_Item' ) ) {
					continue;
				}
				if ( $help_center_item->get_identifier() === 'contact-support' ) {
					unset( $help_center_items[ $index ] );
					break;
				}
			}
		...
		public function maps_api_browser_key_notification() {
			...
			$notification        = new Yoast_Notification(
				...
				sprintf( __( '%1$s needs a %2$s browser key to show %2$s on your website. You haven\'t set a %2$s browser key yet. Go to the %3$s%1$s API keys page%4$s to set the key, or %5$svisit the knowledge base%4$s for more information.', 'yoast-local-seo' ),
					'Yoast SEO: Local',
					'Google Maps',
			...
			if ( empty( $options['api_key_browser'] ) ) {
		...
		public function maps_api_server_key_notification() {
			...
			$notification        = new Yoast_Notification(
				/* translators: %1$s expands to Yoast SEO: Local, %2$s expands to Google Maps,%3$s expands to a link open tag to the settings page, %4$s expands to the closing tag for the link(s) to the settings page and %5$s expands to the opening tag for the link to the knowledge base article */
				sprintf( __( '%1$s needs a %2$s server key to calculate the geographical location of addresses. You haven\'t set a %2$s server key yet. Go to the %3$s%1$s API keys page%4$s to set the key, or %5$svisit the knowledge base%4$s for more information.', 'yoast-local-seo' ),
					'Yoast SEO: Local',
					'Google Maps',
					'<a href="' . admin_url( 'admin.php?page=wpseo_local#top#api_keys' ) . '">',
					'</a>',
