...
if ( ! class_exists( 'WPSEO_Local_Taxonomy' ) ) {
	...
	class WPSEO_Local_Taxonomy {
		...
		function term_seo_form( $term ) {
			...
			echo ' <button id="remove_marker" class="remove_custom_image button button-secondary wpseo-local-hide-button' . ( ( false == $show_marker ) ? ' hidden' : '' ) . '" data-id="custom_marker" style="color: #f00;">' . __( 'Remove marker', 'yoast-local-seo' ) . '</button>';
