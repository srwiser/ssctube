...
function wpseo_local_show_address( $atts ) {
	...
	*/
	...
	if ( ! empty( $address_format_output ) && true === $atts['hide_name'] && true === $atts['oneline'] ) {
	...
	if ( $location_array_str != '' ) {
		...
		$wpseo_map .= '<script type="text/javascript">
			...
			function wpseo_map_init' . ( ( $map_counter != 0 ) ? '_' . $map_counter : '' ) . '() {
				...
				map_' . $map_counter . ' = wpseo_show_map( location_data, ' . $map_counter . ', ' . $center_lat . ', ' . $center_long . ', ' . $atts['zoom'] . ', "' . $atts['map_style'] . '", "' . $atts['scrollable'] . '", "' . $atts['draggable'] . '", "' . is_admin() . '" );
	...
	if ( $wpseo_enqueue_geocoder ) {
		...
		if ( ! empty( $options['api_key_browser'] ) ) {
			$google_maps_url = add_query_arg( array(
				'key' => esc_attr( $options['api_key_browser'] ),
			), $google_maps_url );
		}
...
function wpseo_local_do_upgrade( $db_version ) {
	...
	if ( version_compare( $db_version, '3.3.1', '<' ) ) {
		// If upgrading from below 3.3.1, set defaults.
		yoast_wpseo_local_set_defaults();
	}
